...
dump_chead(FILE *f, const char *parsername, int nofastpath, size_t cbssz)
{
  ...
  fprints(f, "        return 0;\n"); // I think here we don't need to call callbacks
  fprints(f, "      }\n");
  ...
  // FIXME I think here we need to call callbacks for the dropped data
  fprints(f, "          ctx->backtrackstart = ctx->backtrackmid; // FIXME correct?\n"); // FIXME this may interact poorly with cb sys
  fprints(f, "        }\n");
  fprints(f, "      }\n");
  fprints(f, "    }\n");
  ...
  if (cbssz || 1)
  {
    ...
    fprints(f, "          if (cbmask & (1ULL<<bitoff))\n");
    fprints(f, "          {\n");
    fprints(f, "            cbr = cbtbl[bitoff](buf, i, (ctx->start_status & (1ULL<<bitoff)) ? 0 : YALE_FLAG_START, pctx);\n");
  ...
  if (cbssz || 1)
  {
    ...
    fprints(f, "            if (cbmask & (1ULL<<bitoff))\n");
    fprints(f, "            {\n");
    fprints(f, "              cbr = cbtbl[bitoff](buf, i + 1, (ctx->start_status & (1ULL<<bitoff)) ? 0 : YALE_FLAG_START, pctx);\n");
  ...
  fprints(f, "      if (cbmask & (1ULL<<bitoff))\n");
  fprints(f, "      {\n");
  fprints(f, "        cbr = cbtbl[bitoff](buf, sz, (ctx->start_status & (1ULL<<bitoff)) ? 0 : YALE_FLAG_START, pctx);\n");
