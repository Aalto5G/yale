...
dump_chead(FILE *f, const char *parsername)
{
  ...
  fprints(f, "      {\n");
  fprints(f, "        cbtbl[cbs[st->acceptid]](buf, i, start, pctx);\n");
  fprints(f, "      }\n");
  fprints(f, "      if (cb1 != PARSER_UINT_MAX && st->accepting)\n");
  fprints(f, "      {\n");
  fprints(f, "        cbtbl[cb1](buf, i, start, pctx);\n");
  fprints(f, "      }\n");
  fprints(f, "      return i;\n");
  fprints(f, "    }\n");
  ...
  fprints(f, "        if (cbs && st->accepting && cbs[st->acceptid] != PARSER_UINT_MAX)\n");
  fprints(f, "        {\n");
  fprints(f, "          cbtbl[cbs[st->acceptid]](buf, i + 1, start, pctx);\n");
  fprints(f, "        }\n");
  fprints(f, "        if (cb1 != PARSER_UINT_MAX && st->accepting)\n");
  fprints(f, "        {\n");
  fprints(f, "          cbtbl[cb1](buf, i + 1, start, pctx);\n");
  fprints(f, "        }\n");
  ...
  fprints(f, "  {\n");
  fprints(f, "    cbtbl[cbs[st->acceptid]](buf, sz, start, pctx);\n");
  fprints(f, "  }\n");
  fprints(f, "  if (st && cb1 != PARSER_UINT_MAX && st->accepting)\n");
  fprints(f, "  {\n");
  fprints(f, "    cbtbl[cb1](buf, sz, start, pctx);\n");
  fprints(f, "  }\n");
  fprints(f, "  *state = PARSER_UINT_MAX;\n");
